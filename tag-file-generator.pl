...
my $emir_text   = '//# generated by  1.3 for tag(s): ';
...
foreach my $tag (@tags) {
	...
	if ($tag =~ /^title$/) {   # fix visible text
		$code =~ s#(</title>)#EMiR - Event Mappings in Reality ( 1.3)$1#;
...
foreach my $tag (@tags) {
	...
	# TODO: code only works as expected with a single frame* argument only
